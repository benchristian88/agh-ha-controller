...
| Distinct Change History page | Implemented | Immutable revision table/detail/snapshot, revision-to-revision semantic comparison, deployment joins, and historical-revision rollback deployment | Friendly actor-name resolution is not exposed by the current read model |
