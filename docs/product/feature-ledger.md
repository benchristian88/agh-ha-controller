...
| DHCP management | Implemented; Phase 8A presentation complete | Inventory, dynamic observed leases, node-specific config/static leases, single-active validation, disable-before-enable ordering, idempotent reconciliation, controller-mediated interface discovery, and audited non-mutating active-DHCP checks | Controlled-network handoff exercise and packaged retry of the reported disabled-node case |
