...
| Release 0.4 UI | Implemented | Seven nested settings routes using one primary navigation, responsive accessible forms, schema gate, partial refresh, TLS and DHCP safeguards with in-card node headings; rewrites, clients, and static leases avoid secure-context-only UI identifiers | Browser accessibility/visual smoke |
