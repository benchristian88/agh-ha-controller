import type {
  ...
  BlocklistPresentationNode,
  FilterListMetadata,
} from "../../lib/types";

export type BlocklistDraftState =
  | "unchanged"
  | "added"
  | "removal_pending"
  | "observed_only";
export type BlocklistApplicationState =
  | "applied"
  | "pending"
  | "mixed"
  | "disabled"
...
export interface BlocklistNodeState {
  nodeId: string;
  nodeName: string;
  status: BlocklistPresentationNode["status"];
  fetchedAt?: string;
  errorCode?: string;
...
export interface BlocklistRow {
  ...
  active: boolean;
  portable: boolean;
  draftState: BlocklistDraftState;
  applicationState: BlocklistApplicationState;
  name: string;
  mixedName: boolean;
  ruleCount?: number;
...
export function validateBlocklistURL(
  ...
  if (
    ...
    return "This blocklist URL is already in the draft.";
  }
  return undefined;
}

function draftState(desired: boolean, active: boolean): BlocklistDraftState {
...
function applicationState(
  desired: boolean,
  nodes: readonly BlocklistNodeState[],
): BlocklistApplicationState {
...
function fallbackName(rawURL: string): string {
  try {
    ...
    return parsed.hostname || "New blocklist";
  } catch {
